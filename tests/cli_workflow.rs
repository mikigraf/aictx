...
fn add_personal_context(root: &Path) {
    ...
    run_ok(ctxlane(root).args(["profile", "add", "claude", "personal", "--auth", "api-key"]));
    run_ok(ctxlane(root).args(["context", "add", "personal", "--claude", "claude:personal"]));
}
...
fn lifecycle_mutations_before_init_report_not_initialized_without_creating_layout() {
    ...
    for arguments in [
        ...
        assert!(output.stdout.is_empty());
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(stderr.contains("ctxlane is not initialized"));
        assert!(stderr.contains("ctxlane init"));
...
fn bare_invocation_refuses_non_terminal_io_without_ansi_output() {
    ...
    let output = ctxlane(&root)
        .output()
        .unwrap_or_else(|error| panic!("run bare ctxlane: {error}"));
...
fn use_updates_only_mutable_state_and_bindings_take_precedence() {
    ...
    run_ok(ctxlane(&root).args([
        ...
        "subscription-token",
        "--secret-ref",
        "keyring://ctxlane/claude-personal",
    ]));
    run_ok(ctxlane(&root).args([
        "profile",
        "add",
    ...
    let output = run_ok(ctxlane(&root).current_dir(&company).arg("current"));
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "personal");

    fs::write(
        company.join(".ctxlane.toml"),
        "secret_command='curl attacker'",
    )
    .unwrap_or_else(|error| panic!("write malicious repo config: {error}"));
    fs::write(
        company.join(".aictx.toml"),
        "secret_command='curl legacy-attacker'",
    )
    .unwrap_or_else(|error| panic!("write malicious legacy repo config: {error}"));
    let output = run_ok(ctxlane(&root).current_dir(&company).arg("current"));
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "personal");
}
...
fn binding_errors_are_actionable_and_deleted_directories_can_be_unbound() {
    ...
    run_ok(ctxlane(&root).current_dir(temporary.path()).args([
        "bind",
        "company/project",
        "personal",
    ]));
    ...
    let bindings = run_ok(ctxlane(&root).arg("bindings"));
    assert_eq!(
...
fn subscription_auth_is_provider_neutral_and_persists_vendor_native_modes() {
    ...
    for (provider, name, auth) in [
        ...
        run_ok(ctxlane(&root).args(["profile", "add", provider, name, "--auth", auth]));
    }

    let claude = run_ok(ctxlane(&root).args(["profile", "show", "claude:canonical"]));
    ...
    for profile in [
        ...
        let codex = run_ok(ctxlane(&root).args(["profile", "show", profile]));
    ...
    let rejected = ctxlane(&root)
        .args([
            ...
            "subscription-token",
            "--secret-ref",
            "keyring://ctxlane/must-not-be-used",
        ])
        .output()
...
fn profile_help_uses_the_provider_neutral_subscription_mode() {
    let temporary = TempDir::new().unwrap_or_else(|error| panic!("tempdir: {error}"));
    let root = temporary.path().join("ctxlane");
    let help = run_ok(ctxlane(&root).args(["profile", "add", "--help"]));
...
fn account_profile_changes_require_explicit_non_interactive_confirmation() {
    ...
    let output = run_ok(ctxlane(&root).arg("current"));
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "personal");

    run_ok(ctxlane(&root).args(["--non-interactive", "use", "work", "--yes"]));
    let output = run_ok(ctxlane(&root).arg("current"));
...
fn doctor_fails_readiness_when_a_wif_identity_source_is_missing() {
    let temporary = TempDir::new().unwrap_or_else(|error| panic!("tempdir: {error}"));
    let root = temporary.path().join("ctxlane");
    let fake_claude = copy_ctxlane_as_vendor(temporary.path(), "claude");
    ...
    let ready = run_ok(ctxlane(&root).arg("--claude-bin").arg(&fake_claude).args([
        "doctor",
        "--provider",
        "claude",
        "--json",
    ...
    assert!(report["checks"].as_array().is_some_and(|checks| {
        checks.iter().any(|check| {
            check["level"] == "failure"
                ...
                && check["detail"]
                    .as_str()
                    .is_some_and(|detail| detail.contains("ctxlane profile add"))
        })
    }));
}
...
fn concurrent_metadata_updates_are_serialized_without_lost_writes() {
    ...
    for index in 0..8 {
        ...
        let secret_ref = format!("keyring://ctxlane/{name}");
        let child = ctxlane(&root)
            .args([
                "profile",
                "add",
                "claude",
    ...
    let output = run_ok(ctxlane(&root).args(["profile", "list"]));
...
fn public_command_surface_supports_a_complete_local_lifecycle() {
    ...
    let initialized = run_ok(ctxlane(&root).arg("init"));
    let initialized_stdout = String::from_utf8_lossy(&initialized.stdout);
    assert!(initialized_stdout.contains("Initialized ctxlane metadata."));
    assert!(initialized_stdout.contains("Next: add a profile with `ctxlane profile add --help`."));
    let idempotent = run_ok(ctxlane(&root).arg("init"));
    assert_eq!(
        String::from_utf8_lossy(&idempotent.stdout).trim(),
        "ctxlane is already initialized; existing metadata was left unchanged."
    );
    let quiet = run_ok(ctxlane(&root).args(["--quiet", "init"]));
    ...
    let profiles = run_ok(ctxlane(&root).args(["profile", "list"]));
    ...
    let shown_profile = run_ok(ctxlane(&root).args(["profile", "show", "claude:personal"]));
    ...
    let contexts = run_ok(ctxlane(&root).args(["context", "list"]));
    ...
    let shown_context = run_ok(ctxlane(&root).args(["context", "show", "personal"]));
    assert!(String::from_utf8_lossy(&shown_context.stdout).contains("Context: personal"));

    let status = run_ok(ctxlane(&root).arg("status"));
    ...
    let referenced_profile = ctxlane(&root)
    ...
    run_ok(ctxlane(&root).arg("unbind").arg(&project));
    let no_bindings = run_ok(ctxlane(&root).arg("bindings"));
    assert_eq!(
    ...
    for shell in ["bash", "zsh", "fish", "powershell"] {
        let environment = run_ok(ctxlane(&root).args(["env", "--shell", shell]));
        let environment = String::from_utf8_lossy(&environment.stdout);
        assert!(environment.contains("CTXLANE_CONTEXT"), "shell={shell}");
        ...
        let init = run_ok(ctxlane(&root).args(["shell-init", shell]));
    ...
    for shell in ["bash", "elvish", "fish", "powershell", "zsh"] {
        let completion = run_ok(ctxlane(&root).args(["completions", shell]));
        let completion = String::from_utf8_lossy(&completion.stdout);
        assert!(!completion.trim().is_empty(), "shell={shell}");
        assert!(completion.contains("ctxlane"), "shell={shell}");
    }

    run_ok(ctxlane(&root).args(["context", "remove", "personal"]));
    run_ok(ctxlane(&root).args(["profile", "remove", "claude:personal"]));
    run_ok(ctxlane(&root).args(["profile", "remove", "codex:personal"]));
    let no_contexts = run_ok(ctxlane(&root).args(["context", "list"]));
    assert_eq!(
        String::from_utf8_lossy(&no_contexts.stdout).trim(),
        "No contexts configured."
    );
    let no_profiles = run_ok(ctxlane(&root).args(["profile", "list"]));
    assert_eq!(
...
fn repository_local_layout_override_is_refused_before_initialization() {
    ...
    let safe_root = temporary.path().join("safe-ctxlane");
    let inherited_root = project.join("inherited-ctxlane");
    let legacy_inherited_root = project.join("inherited-aictx");
    run_ok(
        ctxlane(&safe_root)
            .current_dir(&project)
            .env("CTXLANE_ROOT", &inherited_root)
            .env("AICTX_ROOT", &legacy_inherited_root)
            .arg("init"),
    );
    ...
    assert!(
        !legacy_inherited_root.exists(),
        "legacy inherited root controls must be ignored"
    );
}
...
fn recreating_removed_profile_cannot_reuse_old_state_or_default_keyring_item() {
    ...
    let root = temporary.path().join("ctxlane");
    run_ok(ctxlane(&root).arg("init"));
    run_ok(ctxlane(&root).args(["profile", "add", "claude", "reusable", "--auth", "api-key"]));
    ...
    let refused_delete = ctxlane(&root)
        .args([
            "--non-interactive",
            "profile",
            "remove",
