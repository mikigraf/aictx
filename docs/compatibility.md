...
The config-v2 automation policy is operator-owned foundation and defaults to `eligible = false`. This release has no supported lease service, automation MCP server, listener, or controller runtime. It contains a crate-internal Linux/macOS schema-v3 SQLite store foundation for request/replay, lifecycle mutation, four-dimensional capacity, audit, retention pruning, lossless lease snapshots, redacted recovery enumeration, and terminal-only prior-generation recovery, plus a separate sealed authority/configuration, canonical Ed25519 proof, and caller-evidence checkpoint. Frozen schema-v1 is still migrated through v2; a synthetic resolved v1 row fails closed because v1 had no trustworthy persisted monotonic high-water or renewal anchor from which authority could be reconstructed. The store mutation API is private to its module and proves neither current config/readiness nor authority/controller-epoch freshness. There is still no process writer/reconciler, launch gate, cross-generation resume, or complete CTX-LEASE-007 recovery. Neither foundation is opened by ordinary commands or a production-recovery/authority claim. Ordinary CLI/TUI account switching and supported local vendor workflows remain standalone, with no ASF, Runmill, service, controller, MCP, lease-store, or authority-file dependency. ASF and Runmill remain optional examples of the controller-neutral future interfaces.
