...
The separate sealed-store tests prove the internal durable request/replay and lifecycle transaction boundary, mutation-on-error persistence, capacity accounting and resource-lock compatibility, bounded retention/pruning, and a conservative terminal-only recovery gate on supported local filesystems. They do not prove that the policy or authority input is fresh: the raw mutation methods remain store-private, and a later facade must combine message-scoped authentication, verified work-order and prepared-authority configuration/epoch binding, and post-fence current profile/readiness evaluation. Together these foundations still do not provide a listener, per-frame request authentication, supported lease service or authority, process writer/reconciler, provider harness, controller, automation MCP server, cross-generation resume, or complete crash recovery.
