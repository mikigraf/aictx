...
| Automation store foundation | `src/automation/store/**` | owner-private SQLite creation, defensive connection settings, exact frozen-v1-to-v2 migration, schema and installation binding, service locking, recovery typestate, atomic request/replay/refusal/audit records, lossless monotonic high-water snapshots, all-eight-status replay, audit-chain semantics, redacted keyset recovery evidence, replay-retention bounds, corruption and crash-retry handling, extension denial, and unsupported-platform zero-filesystem refusal; no public service, lifecycle writer, cross-generation resume, process reconciliation, or pruning |
