...
| Sealed automation authority | `src/automation/authority/**`, `src/automation/attestation/**` | bounded owner-private authority-file loading, installation/configuration binding, exact trust roots/controller scopes/rates/capacities, strict canonical Ed25519 verification and redaction, unforgeable proof binding, macOS explicit local-development/unqualified evidence, Linux connection-origin parser/matcher/snapshot negatives plus native `SO_PEERPIDFD`, `SOCK_SEQPACKET`, `SO_PASSCRED`, exact per-record credential, truncation, ancillary-rights, and payload-binding checks, and unsupported-platform zero-filesystem refusal; no listener, service, lease authority, or CLI surface |
...
| Automation store foundation | `src/automation/store/**` | owner-private SQLite creation, defensive connection settings, exact frozen-v1/v2 qualification and forward schema-v3 migration, schema and installation binding, service locking, recovery typestate, atomic request/replay/lifecycle/audit mutation including commit-on-domain-error, lossless monotonic high-water snapshots, four-dimensional capacity, exclusive/shared resource-guard compatibility, all-eight-status replay, audit-chain semantics, redacted keyset recovery evidence, terminal-only prior-generation recovery, bounded replay retention, audited pruning, corruption and crash-retry handling, extension denial, and unsupported-platform zero-filesystem refusal; no public/authenticated service facade, policy/controller freshness proof, cross-generation resume, process writer/reconciler, launch gate, or production recovery claim |
| Profile automation fence | `src/config/profile_fence.rs`, `src/config/profile_fence/**`, `src/config/storage/tests.rs` | durable canonical marker creation and recovery, exact identity/content/inode binding, symlink/hardlink/mode refusal, close-on-exec descriptors, nonblocking lifecycle conversion, representative/current/historical same-provider alias-set retention, retryable contention versus hard deferred holders, injected pre/post-unlink cleanup failures, exclusive/shared per-lease resource-lock compatibility, and unsupported-target zero-filesystem behavior |
...
| CLI lifecycle | `tests/cli_workflow.rs`, `tests/error_contract.rs`, `tests/standalone_automation_boundary.rs` | the public binary, plain initialization, non-interactive guided refusal, profile/context lifecycle, strict Codex WIF enrollment and unqualified-runtime refusal, bindings, status, doctor readiness/JSON and count-only automation-policy visibility, shell output, completions, stable exit categories, recovery hints, locking, local filesystem policy, ordinary-command independence from invalid would-be authority and automation-store files, and crash-marker refusal for selected profile use/export/mutation without SQLite access while metadata-only and unrelated-profile operations remain available |
...
The automation wire/schema and pure policy/lease-domain tests prove data shape, authority intersection, replay, deadline, fencing, renewal, and state-machine invariants. The sealed authority tests additionally prove the strict file/configuration boundary, canonical Ed25519 proof primitive, exact controller scoping, and platform-specific evidence classifications. On macOS, the end-to-end internal proof path requires both configuration acknowledgement and runtime opt-in and signs an exact `local-development` authorization; it is always development-unqualified. On Linux, native CI exercises kernel peer credentials and requires atomic `SO_PEERPIDFD`; pure parser and matcher negatives cover duplicate process IDs, exact live states, credential changes, executable path/digest/device/inode changes, unified-cgroup syntax and exact configured path, while a one-field-at-a-time test covers every retained executable metadata field. Connection-only evidence remains verifier-ineligible. The Linux record tests require a close-on-exec `SOCK_SEQPACKET` channel, verified `SO_PASSCRED`, exactly one matching non-truncated `SCM_CREDENTIALS` item, closure and rejection of `SCM_RIGHTS`, bounded payloads, and a payload-specific proof binding. The native Windows job tests the shared unsupported-target zero-filesystem refusal path; other unsupported targets are not qualified.

The separate sealed-store tests prove the internal durable request/replay and lifecycle transaction boundary, mutation-on-error persistence, capacity accounting and resource-lock compatibility, bounded retention/pruning, and a conservative terminal-only recovery gate on supported local filesystems. They do not prove that the policy or authority input is fresh: the raw mutation methods remain store-private, and a later facade must combine message-scoped authentication, verified work-order and prepared-authority configuration/epoch binding, and post-fence current profile/readiness evaluation. Together these foundations still do not provide a listener, socket-discovery/acceptance path, supported lease service or authority, process writer/reconciler, provider harness, controller, automation MCP server, cross-generation resume, or complete crash recovery. The sealed record receiver is not exposed to a client and cannot mutate the store.
...
vendor backport must qualify that exact kernel. Also run the Linux-only
authenticated-record tests:

```bash
cargo test --locked --lib \
  automation::attestation::linux::channel::tests
```

These tests prove socket-option availability, retained pidfd identity, and the
sealed `SOCK_SEQPACKET`/`SO_PASSCRED` record checks on that host. They do not
prove private socket placement, listener behavior, service composition, or
