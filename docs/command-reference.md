...
Migration is copy-only and explicit. `--dry-run` validates the complete source and reports non-secret counts without creating target files. A completed migration rewrites managed profile state paths but preserves existing keyring references and leaves the old metadata, vendor state, and credentials unchanged. Explicit target roots require `--from-root`.
