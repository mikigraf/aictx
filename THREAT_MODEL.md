...
The current code is a local CLI and TUI, not a production automation identity plane. It has no supported identity-lease service, production MCP server, listener or request-framing gate, complete durable lease recovery, or lease-enforced provider harness. A sealed crate-internal authority checkpoint can read and validate a separate operator file, verify canonical Ed25519 work-order proofs, produce explicitly unqualified macOS local-development evidence, and attest the process that opened a Linux connection. A separate sealed schema-v3 SQLite foundation can durably record requests, replay bindings, lease lifecycle transitions, four-dimensional capacity, retention pruning, and audit events; preserve lossless lease clock snapshots; reconstruct every status on exact replay; and enumerate redacted, explicitly non-resumable recovery candidates while prior state blocks readiness. Its mutation methods are private to the store module and prove neither current policy/readiness nor controller-configuration freshness; it has no process writer or reconciler. Neither foundation is wired to the CLI, TUI, a listener, or each other, and neither grants lease, session, execution, or production authority. `--trusted-runner` is an assertion for existing CLI flows; it is not caller attestation and never grants production automation authority.
...
The controls in this table are production requirements, not a claim that the current implementation is qualified. The sealed authority code implements strict configuration and signature primitives, macOS development-only evidence, Linux connection-origin attestation, and an unwired `SOCK_SEQPACKET`/`SO_PASSCRED` authenticated-record receiver whose proof owns and binds the exact payload; the sealed store implements owner-private lifecycle/capacity/retention transactions and a conservative terminal-only recovery gate. There is still no listener, socket-discovery/acceptance path, lease-authority integration, policy/controller freshness facade, process reconciliation, harness isolation, cross-generation resume, or native deployment qualification. The detailed authority matrix and release gates are in [Automation identity plane](docs/automation-identity-plane.md).

| Threat | Consequence | Required control before production | Residual risk |
| --- | --- | --- | --- |
| controller impersonation | an unrelated same-user process acquires model-provider authority | Linux private service channel authenticated with atomic `SO_PEERPIDFD`, peer UID/GID, a retained live process identity, trusted executable canonical path/digest/snapshot, and expected protected cgroup v2/systemd membership; every accepted record must additionally carry exactly matching `SCM_CREDENTIALS` obtained with `SO_PASSCRED` | the sealed receiver enforces the record check but has no listener or lease-authority consumer; delegated connected descriptors are rejected by opener/writer mismatch, while loader/libraries, environment/arguments, process memory/ptrace, unusual filesystem semantics, root, or compromise of the trusted controller/service manager/kernel remain decisive |
...
| unsupported host weakens caller identity | deployment silently omits required peer and process binding | Linux with `SO_PEERPIDFD` support is the only production target; macOS evidence is explicit local-development and unqualified; Windows and other unsupported authority loaders fail before path derivation or filesystem access | upstream Linux 6.5 or a qualified backport, host configuration, native authenticated-record behavior, and private-channel deployment still require protected qualification |
...
- treating Linux connection-origin attestation, or the sealed but unwired authenticated-message proof, as lease authority without the future service composition;
