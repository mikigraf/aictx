...
- Added a sealed, crate-internal SQLite lease-store and recovery-gate foundation on Linux and macOS. It atomically records initial requests, replay bindings, refusals, and append-only audit events, but it is not a service and does not yet activate leases, manage processes, or claim production recovery. Windows and other unsupported targets refuse this store boundary before filesystem access.
- Advanced the sealed lease store to schema v2 with exact frozen-v1 qualification, lossless monotonic high-water and renewal-anchor snapshots, full eight-status replay reconstruction, semantic audit-chain validation, and keyset-paginated redacted recovery evidence. Cross-generation resume and lifecycle mutation remain deliberately unavailable.
- Advanced the sealed store to schema v3 with store-private transactional activation, renewal acknowledgement, close/revoke/error/expiration, four-dimensional capacity, terminal-only prior-generation recovery, and audited seven-day retention pruning. Fail-closed domain errors commit their resulting clock or terminal transition. These mechanics remain unwired and do not authenticate a caller, prove current policy/controller freshness, reconcile or launch processes, resume an old generation, or expose service authority.
- Added a durable per-profile automation fence that precedes unresolved requests, retains legacy alias and UID lifecycle locks, and holds an exclusive or rigorously qualified shared resource lock per active lease. Ordinary standalone profile use and mutation refuse a fenced profile without opening the automation database; unrelated and metadata-only workflows remain independent.
- Added a separate sealed, crate-internal `config/automation-authority.toml` checkpoint with a strict closed version-1, owner-private 1 MiB read-only loader, installation/host binding, exact service and controller scopes/ceilings, canonical Ed25519 work-order verification primitives, redacted views, macOS local-development evidence, and Linux connection-origin process/deployment attestation. It is unwired and exposes no operator command, listener, lease authority, or public API; unsupported targets refuse before authority-path filesystem access.
...
- Added standalone boundary regressions that run ordinary CLI and real dashboard PTY flows beside deliberately invalid would-be authority and lease-store files and prove neither file affects or is changed by standalone operation.
...
- Kept Linux peer/process evidence verifier-ineligible and classified as connection-origin attestation only. Runtime requires atomic `SO_PEERPIDFD` support, and any future listener must add `SO_PASSCRED` plus matching per-frame `SCM_CREDENTIALS` before a Linux request can carry authority; macOS remains explicitly opt-in and development-unqualified.
